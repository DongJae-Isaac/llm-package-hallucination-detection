...
OUTPUT_CSV  = r"D:\Programming\@@Sec\paper_prompts_expanded_v2_out_codellama7b_final.csv"
PROGRESS_CSV = r"D:\Programming\@@Sec\progress_checkpoint_codellama7b.csv" 
...
def ollama_chat(system_prompt: str, user_prompt: str):
    """Ollama chat API 호출 + usage/latency 반환 (오류 파싱 및 재시도)"""
        ...
        try:
            ...
            if hasattr(e, 'response') and e.response is not None:
                try:
                    ...
                    server_response_text = error_details.get("error", "")
            ...
            if "context length" in server_response_text.lower():
                ...
                if match:
                    ...
                    # 다음 루프(재시도)를 위해 payload의 num_predict 값을 덮어씁니다.
                    payload["options"]["num_predict"] = new_max_tokens
                    print(f"    -> Retrying with new num_predict: {new_max_tokens}")
